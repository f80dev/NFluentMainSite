...
export class AppsComponent implements OnInit {
  ...
  async refresh_keys(filter="bob,alice,dan"){
    this.keys=[];
    let keys:any[]=await this.api.init_keys(true,"","","elrond-devnet")
    for(let k of keys){
      if(k && filter.indexOf(k.name || "")>-1){
        k.label=k.name;
        this.keys.push(k);
      }
    }
  ...
  open_app(app: any) {
    if(this.sel_key){
      ...
      for(let k of Object.keys(app.test_url)){
        ...
        if(v=="address")v=this.sel_key.address;
        app.test_url[k]=v
      }
      let p=setParams(app.test_url)
      open(app.url+"?"+p,"App")
    }else{
      open(app.url,"App")
    }
